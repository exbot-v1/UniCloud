...
export const UploadModal: React.FC<UploadModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/75 backdrop-blur-xs flex items-center justify-center p-4 animate-in fade-in duration-150">
      <div className="w-full max-w-xl bg-[#12161f] rounded-2xl p-6 shadow-2xl border border-[#262c36] space-y-5 max-h-[92vh] overflow-y-auto text-slate-100">
        {/* Modal Header */}
        <div className="flex items-start justify-between border-b border-[#262c36] pb-3.5">
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="flex items-center gap-2">
                <h3 className="text-base font-bold text-white">Upload & Capacity Router</h3>
                <span className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-cyan-950/70 text-cyan-300 border border-cyan-800/60 uppercase">
                  Phase 4 Active
                </span>
              </div>
              <p className="text-xs text-slate-400">
                Resumable multi-account streaming uploads to Google Drive
              </p>
            </div>
          </div>
          <button
            ...
            className="p-1.5 rounded-xl text-slate-400 hover:text-white hover:bg-[#1a202c] transition-colors disabled:opacity-40"
          >
            <X className="h-5 w-5" />
          </button>
        </div>
        ...
        {/* Tab Navigation */}
        <div className="flex items-center p-1 bg-[#0a0d13] rounded-xl border border-[#262c36]">
          <button
            type="button"
        ...
        {/* ===================================================================== */}
        {/* TAB 1: REAL RESUMABLE FILE UPLOAD (PHASE 4)                           */}
        {/* ===================================================================== */}
        {activeTab === 'upload' && (
          <div className="space-y-4">
            {!isCompleted ? (
              <>
                ...
                {!selectedFile ? (
                  <div
                    ...
                    className="border-2 border-dashed border-[#2b3342] hover:border-cyan-500/60 bg-[#0e1117]/80 hover:bg-[#141923] rounded-xl p-6 text-center cursor-pointer transition-all group"
                  >
                    <input
                      ref={fileInputRef}
                      type="file"
                      ...
                      className="hidden"
                    />
                    <div className="w-12 h-12 rounded-xl bg-cyan-500/10 text-cyan-400 flex items-center justify-center mx-auto mb-3 group-hover:scale-105 transition-transform border border-cyan-500/20">
                  ...
                  <div className="p-3.5 bg-[#0e1117] rounded-xl border border-[#262c36] flex items-center justify-between">
                    ...
                    {!isUploading && (
                      <button
                        ...
                        className="text-xs text-slate-400 hover:text-rose-400 p-1 rounded-lg transition-colors"
                      >
                        Change
                      </button>
                    )}
                ...
                <div className="space-y-3 pt-1">
                  <div>
                    <label className="font-semibold text-slate-300 block text-xs mb-1.5">
                      Upload Routing Mode
                    </label>
                  ...
                  {strategy === UploadRoutingStrategy.MANUAL && (
                    <div className="animate-in fade-in duration-100">
                      <label className="font-semibold text-slate-300 block text-xs mb-1">
                      ...
                      <select
                        ...
                        className="w-full px-3 py-2 bg-[#0e1117] border border-[#262c36] text-white rounded-xl focus:border-cyan-500 focus:outline-hidden text-xs"
                      >
                ...
                {/* Live Progress Bar */}
                {isUploading && (
                  <div className="p-3.5 rounded-xl bg-[#0a0d13] border border-[#262c36] space-y-2.5 animate-in fade-in duration-150">
                    ...
                    <div className="w-full bg-[#1c222d] h-2.5 rounded-full overflow-hidden">
                      <div
                        className="h-full bg-gradient-to-r from-cyan-500 to-teal-400 transition-all duration-200 ease-out"
                ...
                {uploadError && (
                  <div className="p-3 rounded-xl bg-rose-950/40 border border-rose-800/50 text-rose-300 text-xs flex items-start gap-2.5">
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-[11px] text-rose-300/80">{uploadError}</p>
                    </div>
                  </div>
                )}
                ...
                <div className="flex items-center gap-2 pt-2">
                  {!isUploading ? (
                    <>
                      {uploadError ? (
                        <button
                          type="button"
                          onClick={handleRetry}
                          className="flex-1 py-2.5 px-4 bg-amber-500 hover:bg-amber-400 text-slate-950 rounded-xl text-xs font-bold transition-all flex items-center justify-center gap-2"
                        >
                        ...
                        <button
                          ...
                          className="flex-1 py-2.5 px-4 bg-cyan-500 hover:bg-cyan-400 disabled:bg-[#1a202c] disabled:text-slate-500 disabled:cursor-not-allowed text-slate-950 rounded-xl text-xs font-bold shadow-md shadow-cyan-950/50 border border-cyan-400/50 transition-all flex items-center justify-center gap-2 active:scale-[0.99] cursor-pointer"
                        >
                    ...
                    <button
                      type="button"
                      onClick={handleAbort}
                      className="w-full py-2.5 px-4 bg-rose-600/20 hover:bg-rose-600/30 text-rose-300 border border-rose-500/40 rounded-xl text-xs font-bold transition-all flex items-center justify-center gap-2"
                    >
              ...
              /* Success State */
              <div className="p-5 bg-[#0a0d13] rounded-2xl border border-emerald-500/40 text-center space-y-4 animate-in zoom-in-95 duration-200">
                <div className="w-12 h-12 rounded-2xl bg-emerald-500/20 text-emerald-400 border border-emerald-500/30 flex items-center justify-center mx-auto shadow-lg shadow-emerald-950/50">
                  <CheckCircle2 className="h-6 w-6" />
                </div>
                <div>
                  <h4 className="text-base font-bold text-white">Upload Completed!</h4>
                  <p className="text-xs text-slate-400 mt-1">
                    File was successfully streamed and mapped to your virtual cloud filesystem.
                  </p>
                </div>

                <div className="p-3 bg-[#12161f] rounded-xl border border-[#262c36] text-left text-xs space-y-2">
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className="font-mono text-cyan-300">{formatBytes(selectedFile?.size || 0)}</span>
                  </div>
                  {assignedAccount && (
                    <div className="flex justify-between border-t border-[#262c36] pt-1.5">
                ...
                <div className="flex items-center gap-2 pt-2">
                  <button
                    type="button"
                    onClick={handleResetForNewUpload}
                    className="flex-1 py-2 px-3 bg-[#1a202c] hover:bg-[#232b3a] text-slate-300 rounded-xl text-xs font-semibold border border-[#262c36] transition-all"
                  >
                    Upload Another
                  </button>
                  <button
                    type="button"
                    onClick={onClose}
                    className="flex-1 py-2 px-3 bg-cyan-500 hover:bg-cyan-400 text-slate-950 rounded-xl text-xs font-bold transition-all"
                  >
                    Done
                  </button>
                </div>
        ...
        {/* ===================================================================== */}
        {/* TAB 2: CAPACITY ROUTING SIMULATOR (PHASE 0 PRESERVED)                 */}
        {/* ===================================================================== */}
        {activeTab === 'simulate' && (
          <div className="space-y-4 text-xs">
            <div className="p-3 rounded-xl bg-cyan-950/40 border border-cyan-800/50 text-cyan-200 text-xs flex items-start gap-2.5">
              ...
              <div>
                <span className="font-bold">Capacity Routing Simulator:</span> Test how UniCloud's
                routing engine distributes multi-gigabyte files across your accounts before uploading.
              </div>
            </div>
            ...
            <div>
              <div className="flex justify-between font-semibold text-slate-300 mb-1">
                ...
                <span className="text-cyan-400 font-bold">{simSizeMb} MB ({formatBytes(simSizeBytes)})</span>
              </div>
              <input
                type="range"
                min={10}
            ...
            <button
              ...
              className="w-full py-2.5 px-4 bg-cyan-500 hover:bg-cyan-400 text-slate-950 rounded-xl text-xs font-bold shadow-md shadow-cyan-950/50 border border-cyan-400/50 transition-all flex items-center justify-center gap-2 active:scale-[0.99] cursor-pointer"
            >
            ...
            {simRoutingResult && simSelectedAccount && (
              <div className="p-4 rounded-xl bg-[#10141b] border border-[#262c36] space-y-3 text-xs animate-in fade-in slide-in-from-top-2">
                ...
                <p className="text-slate-300 text-[11px]">{simRoutingResult.reason}</p>
                ...
                <div className="p-3 bg-[#161b24] rounded-xl border border-[#262c36] space-y-2">
                  ...
                  <div className="grid grid-cols-2 gap-2 text-[11px] text-slate-400 pt-1.5 border-t border-[#262c36]">
                    <div>
                      ...
                      <span className="font-bold text-white">{formatBytes(simRoutingResult.availableCapacityBeforeBytes)}</span>
                    </div>
                    <div>
                      <span className="text-slate-400 block">Projected After Upload:</span>
                      <span className="font-bold text-teal-300">{formatBytes(simRoutingResult.projectedCapacityAfterBytes)}</span>
                    </div>
                  </div>
                </div>
              </div>
